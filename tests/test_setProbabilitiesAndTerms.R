########################################################################################################################################
# Test setProbabilitiesAndTerms (1)
#
# setProbabilitiesAndTerms should:
#   (a) zero out the probabilities for any terms built using variables outside the current variable set
#   (b) zero out the probabilities for any terms already present by way of a term at a higher term level
#   (c) maintain all other probabilities
#
########################################################################################################################################

test_that("Test setProbabilitiesAndTerms (1)", {

  predictorVars <- c("a","b","c","d")
  ...
  expectedTermProbabilities[[2]] <- c(0,0,0,0,0.5,0,0,0,0,0)
  expectedTermProbabilities[[1]] <- c(0,0,0,0,0)
  
  expect_equal(setProbabilitiesAndTerms(termProbabilities, termOptions, predictorVars, highestTermLevel, nTermsAtLevel)[[1]], expectedTermProbabilities)  
